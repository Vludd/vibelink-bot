...
class InterestRepository:
    ...
    async def get_or_create(self, category: InterestCategory, name: str) -> Interest:
        ...
        existing_interest = await self.get_by_category_and_name(category, name)
        if existing_interest is None:
            raise RuntimeError(
                f"Interest was not inserted and existing interest was not found: "
    ...
    async def get_by_category_and_name(
        ...
        stmt = (
            select(Interest)
            .where(Interest.category == category, Interest.name == name)
        )
