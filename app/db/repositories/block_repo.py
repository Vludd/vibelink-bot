...
class BlockRepository:
    ...
    async def block(self, blocker_id: int, blocked_id: int) -> Block:
        ...
        existing_block = await self.get(blocker_id, blocked_id)
        if existing_block is None:
            raise RuntimeError(
                f"Block was not inserted and existing block was not found: "
    ...
    async def unblock(self, blocker_id: int, blocked_id: int) -> bool:
        ...
        result = await self.session.execute(stmt)
        deleted_block_id = result.scalar_one_or_none()

        return deleted_block_id is not None
